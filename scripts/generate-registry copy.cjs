...
// Списки для спеціальної обробки
const NATIVE_PROXY_COMPONENTS = [
    "button",
    "badge",
    "input",
    "textarea",
...
const DIV_PROXY_COMPONENTS = ["card", "skeleton"];
...
components.forEach(({ name, pascalName }) => {
    // 1. Native Proxy Strategy
    if (NATIVE_PROXY_COMPONENTS.includes(name)) {
        let tagName = "div";
        if (name === "button") tagName = "button";
        if (name === "input") tagName = "input";
        if (name === "textarea") tagName = "textarea";

        content += `    // ${pascalName} (Native Proxy)\n`;
        content += `    ${pascalName}: createNativeComponent('${tagName}', ${pascalName}Comp.${name}Variants),\n`;
        return;
    }

    // 2. Card Strategy (Div Proxy)
    if (DIV_PROXY_COMPONENTS.includes(name)) {
        content += `    // ${pascalName} (Div Proxy)\n`;
        // Тут ми просто беремо всі експорти і перетворюємо їх на DivProxy, бо CardHeader це просто стилізований div
        content += `    ...Object.keys(${pascalName}Comp).reduce((acc, key) => {
             // Пропускаємо variants, якщо є
             if(key.includes('Variants')) return acc;
             // Припускаємо, що компонент це просто div з класами. 
             // Для повної точності краще використовувати Portal, але спробуємо Portal за замовчуванням для надійності
             // @ts-ignore
    ...
    // 3. Default Strategy (Portal) - Найбезпечніша для всього іншого (Dialog, Select, etc)
