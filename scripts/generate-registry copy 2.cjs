...
// Input, Textarea, Checkbox - це просто HTML теги з класами
const NATIVE_SIMPLE = [
    "input",
    "textarea",
    "checkbox",
...
files.forEach((file) => {
    ...
    // Пропускаємо index.ts або інші службові файли
...
const createNativeComponent = (tagName: string, variantFunction?: Function) => {
    const ProxyComponent = (props: any) => {
        ...
        // Спеціальна обробка для void елементів (input, hr)
...
components.forEach(({ name, pascalName }) => {
    ...
    if (DIV_PROXY.includes(name)) {
        ...
        // Для Separator використовуємо "div" (або "hr" якщо хочете, але div безпечніше для shadcn стилів)
